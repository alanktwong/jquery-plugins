/**
 * <p>This is a fork of the store library by AmplifyJS.
 ...
 * AND it does NOT support the userData storageType for IE.</p>
 * <p>This library also depends on the library, JSON.js.<p>
 * 
 * <h2>Usage</h2>
 * 
 * <div>
 *     <pre><code>$.store( string key, mixed value [, hash options ] )</code></pre>
 ...
 * </div>
 * <div>
 *     <pre><code>$.store( string key )</code></pre>
 ...
 * </div>
 * <div>
 *     <pre><code>$.store( string key, null )</code></pre>
 *     Clears key/value pair from the store.
 * </div>
 * <div>
 *     <pre><code>$.store.storageType( string key, mixed value [, hash options ] )</code></pre>
 *     Stores a value for a given key using an explicit storage type, where <code>storageType</code>
 *     is one of the available storage types through amplify.store.
 *     The storage types available by default are listed below.
 * </div>
 * <div>
 *     <pre><code>$.store.storageType( string key )</code></pre>
 *     Gets a stored value based upon key for the explicit storage type.
 * </div>
 * <div>
 *     <pre><code>$.store.storageType()</code></pre>
...
;(function( $, undefined ) {
	...
	for ( var webStorageType in { localStorage: 1, sessionStorage: 1 } ) {
		...
		try {
			...
			window[ webStorageType ].setItem( "__jQuery.store__", "x" );
			window[ webStorageType ].removeItem( "__jQuery.store__" );
